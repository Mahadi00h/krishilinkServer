...
app.get("/crops/latest", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch latest crops", error: error.message });
  }
});
...
app.get("/my-crops/:email", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch user crops", error: error.message });
  }
});
...
app.post("/interests", async (req, res) => {
  try {
    ...
    if (existingInterest) {
      return res
        .status(400)
        .json({ message: "You have already sent an interest for this crop" });
    }
